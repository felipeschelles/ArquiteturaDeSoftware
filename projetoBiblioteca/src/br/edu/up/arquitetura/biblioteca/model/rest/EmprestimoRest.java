...
import br.edu.up.arquitetura.biblioteca.model.negocio.EmprestimoNegocio;
...
public class EmprestimoRest {
		...
		public Response insert(Emprestimo emprestimo, @Context UriInfo uriInfo) throws Exception{
			...
			String id = ""+bc.cadastrar(emprestimo).getId();
		 ...
		 @Produces({ MediaType.APPLICATION_JSON })
		 @Consumes({ MediaType.APPLICATION_JSON })
		 public void devolver(@PathParam("id") int id, Emprestimo emprestimo) throws Exception {
			 
			
		     emprestimo.setId(id);
		     bc.devolver(emprestimo);
		 }
